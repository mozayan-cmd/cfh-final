...
@section('content')
<div class="mb-8">
    <a href="{{ route('payments.index') }}" class="text-black-50 hover:text-off-black mb-4 inline-flex items-center">
...
<div class="card rounded-xl p-6 max-w-3xl mx-auto">
    ...
    <div class="bg-green-500/10 border border-green-500/30 rounded-lg p-4 mb-6">
        <p class="text-green-400">Ready to import <strong>{{ count($results) }}</strong> payment(s)</p>
    </div>
        ...
        <table class="w-full text-sm">
            <thead class="bg-slate-50/60 dark:bg-slate-700/50">
                <tr class="text-black-50">
                    ...
                    <th class="text-left px-4 py-3">Payment For</th>
                </tr>
            </thead>
            <tbody class="divide-y divide-gray-700/50">
                ...
                <tr class="hover:bg-white/5">
                    ...
                    <td class="px-4 py-3 text-red-400 text-right">₹{{ number_format($row['amount'], 2) }}</td>
                    <td class="px-4 py-3 text-off-black">{{ $row['date'] }}</td>
                    <td class="px-4 py-3 text-off-black">{{ $row['mode'] }}</td>
                    <td class="px-4 py-3 text-off-black">{{ $row['source'] }}</td>
                    <td class="px-4 py-3 text-off-black">{{ $row['payment_for'] }}</td>
                </tr>
                @endforeach
            </tbody>
        </table>
    ...
    <form action="{{ route('payments.import.process') }}" method="POST" class="mt-6">
        @csrf
        <div class="flex justify-between items-center">
            <a href="{{ route('payments.import') }}" class="bg-gray-600 hover:bg-gray-700 text-off-black px-6 py-2 rounded-lg">
                Cancel
            </a>
